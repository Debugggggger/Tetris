...
class Square
{
   ...
   {
      return ( x >= 0 && x < anotherTetris.cols && y >= 0 && y < anotherTetris.rows );
   }
...
public class anotherTetris extends Frame implements Runnable, ActionListener
{
   MenuBar mb;
   Menu game, help;
   ...
   public anotherTetris()                                              
   {
      ...
      public void run()
      {
         while (!lost)
         {
            ...
            if (neednewpiece)               
            {
               if (curval > 0)               
               {
                  level = 1 + removeline/10;   // 레벨 상승 조절(삭제되는 라인기준)
               }
               removelines();            
               newpiece();   
               score += curval;
      ...
      public void drawPiece(int m, int what) {
    	  switch (what2)
          {
          case 0:
             //  ㅁㅁmㅁ
